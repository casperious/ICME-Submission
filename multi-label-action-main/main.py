...
def main(args):
    ...
    print(
        "[INFO] Found", str(torch.cuda.device_count()), "GPU(s) available.", flush=True
    )
    device = torch.device(
    ...
    if args.dataset in ["animalkingdom", "charades"]:
        ...
        eval_metric = MultilabelAveragePrecision(
    ...
    if args.model == "convit":
        ...
        from models.timesformerclipinitvideoguide import (
            TimeSformerCLIPInitVideoGuideExecutor,
        )

        executor = TimeSformerCLIPInitVideoGuideExecutor(*model_args)
    elif args.model == "timesformerresidualclipinit":
        from models.timesformerresidualclipinit import (
            TimeSformerResidualCLIPInitExecutor,
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Training script for action recognition"
    )
    parser.add_argument(
        "--seed",
        default=1,
    ...
    parser.add_argument(
        "--epochs", default=100, type=int, help="Total number of epochs"
    )
    parser.add_argument(
        "--dataset",
        default="volleyball",
    ...
    parser.add_argument(
        "--total_length", default=10, type=int, help="Number of frames in a video"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=5,
        type=int,
        help="Test the model every this number of epochs",
    )
    parser.add_argument(
        "--gpu", default="0", type=str, help="GPU id in case of multiple GPUs"
    )
    parser.add_argument(
        "--distributed", default=False, type=bool, help="Distributed training flag"
    )
    parser.add_argument(
        "--test_part", default=6, type=int, help="Test partition for Hockey dataset"
    )
    parser.add_argument(
        "--zero_shot", default=False, type=bool, help="Zero-shot or Fully supervised"
    )
    parser.add_argument(
        "--split", default=1, type=int, help="Split 1: 50:50, Split 2: 75:25"
    )
