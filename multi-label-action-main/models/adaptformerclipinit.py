...
class AdaptFormerCLIPInit(nn.Module):
    def __init__(self, class_embed, num_frames):
        ...
        self.backbone = AdaptFormer(
            all_frames=num_frames, name_ckpt="pretrain_vit_base_1600.pth"
        )
        self.norm = nn.LayerNorm(self.backbone.num_features)
        self.linear1 = nn.Linear(
            in_features=self.backbone.embed_dim, out_features=self.embed_dim, bias=False
        )
...
class AdaptFormerCLIPInitExecutor:
    def __init__(
        ...
        self.scheduler = CosineAnnealingWarmRestarts(
            self.optimizer, T_0=10, eta_min=1e-8
        )
    ...
    def _construct_optimizer(self):
        ...
        return Adam(optim_params, lr=0.01, weight_decay=0.0)
    ...
    def _train_epoch(self, epoch):
        ...
        for data, label in self.train_loader:
            data, label = data.to(self.gpu_id, non_blocking=True), label.to(
    ...
    def train(self, start_epoch, end_epoch):
        for epoch in range(start_epoch, end_epoch):
            ...
            if (epoch + 1) % self.test_every == 0:
                ...
                if (self.distributed and self.gpu_id == 0) or not self.distributed:
                    print(
                        "[INFO] Evaluation Metric: {:.2f}".format(eval * 100),
                        flush=True,
                    )
